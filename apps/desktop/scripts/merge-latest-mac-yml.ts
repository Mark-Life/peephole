...
/** Narrow an unknown parsed YAML entry to a well-formed update-file record. */
...
/** Parse + validate an electron-builder mac update manifest from YAML text. */
const parseManifest = (source: string, path: string): UpdateManifest => {
  ...
  const files = Array.isArray(raw.files)
    ? raw.files.filter(isUpdateFileEntry)
    : [];
  if (
...
/** Serialize a merged manifest to the exact shape electron-updater expects. */
...
if (!(primaryPath && secondaryPath && outPath)) {
  throw new Error(
    "Usage: merge-latest-mac-yml.ts <primary.yml> <secondary.yml> <out.yml>"
  );
}

const primary = parseManifest(await Bun.file(primaryPath).text(), primaryPath);
const secondary = parseManifest(
  await Bun.file(secondaryPath).text(),
  secondaryPath
);
...
console.log(
  `Merged ${primaryPath} + ${secondaryPath} -> ${outPath} (${files.length} files)`
);
